...
到 [Releases](https://github.com/sw1313/rclone-android/releases/latest) 下载 APK。当前版本 **v1.0.6**，通用包含 arm64 / armeabi-v7a / x86_64，体积较大。覆盖安装即可，不必卸载。
...
首次打开请授予「所有文件访问」。通知只在下载/上传时用来显示进度条（类似浏览器下载），完成后消失。不必开省电「无限制」、自启动或定位。
...
- 本应用不再常驻前台服务。无 Root 的文件传输才短暂拉起 `dataSync` 进度通知，传完即停
